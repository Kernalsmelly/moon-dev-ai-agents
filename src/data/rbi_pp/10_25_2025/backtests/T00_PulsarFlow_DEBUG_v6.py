...
class PulsarFlow(Strategy):
    ...
    def next(self):
        ...
        print(f"🌙 Moon Dev: Bar update - Close: {close:.2f}, MFI: {self.mfi.iloc[current_i]:.2f}, ADX: {self.adx.iloc[current_i]:.2f} 📊")
        ...
        if not self.position:
            ...
            if close > self.ema.iloc[current_i] and self.mfi.iloc[current_i] <= self.oversold:
                print(f"🌙 Moon Dev: Potential LONG: MFI {self.mfi.iloc[current_i]:.2f} <= {self.oversold}, Close {close:.2f} > EMA {self.ema.iloc[current_i]:.2f} 🔍")
            ...
            if close < self.ema.iloc[current_i] and self.mfi.iloc[current_i] >= self.overbought:
                print(f"🌙 Moon Dev: Potential SHORT: MFI {self.mfi.iloc[current_i]:.2f} >= {self.overbought}, Close {close:.2f} < EMA {self.ema.iloc[current_i]:.2f} 🔍")
