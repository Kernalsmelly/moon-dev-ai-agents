...
class ClimaxReversal(Strategy):
    ...
    def get_consec_down(self):
        ...
        while True:
            prev_pos = pos - 1
            if prev_pos < -len(self.data):
                break
            if self.data.Close[pos] >= self.data.Close[prev_pos]:
                break
            count += 1
            pos = prev_pos
        return count
    ...
    def next(self):
        ...
        # Time-based exit
        if self.position and self.entry_bar is not None and (len(self.data) - 1 - self.entry_bar) > self.max_hold_bars:
            self.position.close()
        ...
        if gap_size_pct > 0.0005:  # 0.05% threshold for debug
            ...
            print(f"🌙 Moon Dev Debug: Bar {len(self.data)-1}, Gap%: {gap_size_pct*100:.3f}, Down?: {is_gap_down}, Consec down: {self.get_consec_down()}, up: {self.get_consec_up()}, "
                  f"Gap close vs SMA: {gap_close} vs {self.sma[-2]:.2f}, RSI[i-1]: {self.rsi[-2]:.1f}, "
