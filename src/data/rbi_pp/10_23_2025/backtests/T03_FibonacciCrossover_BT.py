...
class FibonacciCrossover(Strategy):
    ...
    def init(self):
        ...
        self.harmonic_confluence_long = abs(close - self.ema21) / self.ema21 < self.confluence_threshold
        self.harmonic_confluence_short = abs(close - self.ema21) / self.ema21 < self.confluence_threshold
    ...
    def next(self):
        ...
        if (not self.position and
            crossover(self.ema5, self.ema13) and
        ...
        # Short Entry
              ...
              crossunder(self.ema5, self.ema13) and
              self.ema21[-1] < self.ema55[-1] and
              current_rsi < 60 and
              current_close < self.sma200[-1] and
              current_volume > self.volume_ma[-1] and
