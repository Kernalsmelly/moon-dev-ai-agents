...
async def wait_for_finalized(brain: MarketBrain, signature: str, timeout: int = 120):
    """Poll signature status using the MarketBrain shield until finalized or timeout.

    Uses `getSignatureStatuses` RPC via `brain._call_rpc` and interprets common
    response shapes. Returns True when `confirmationStatus == 'finalized'`.
    """
    start = time.time()
    while True:
        try:
            resp = await brain._call_rpc('getSignatureStatuses', [[signature]])
            # Parse common shapes: { result: { value: [ { confirmationStatus: 'finalized' } ] } }
            try:
                if isinstance(resp, dict):
                    arr = resp.get('result', {}).get('value') or resp.get('value') or resp.get('result')
                else:
                    arr = resp
                status = None
...
async def main(count: int):
    ...
    # Use MarketBrain's RPC shield for airdrop / balance operations so calls
    # are subject to the same rotation/blacklist policies as trading flows.
    brain = MarketBrain(rpc=rpc)

    # show starting balance
    try:
        bal_resp = await brain._call_rpc('getBalance', [str(pub)])
        bal = bal_resp.get('result', {}).get('value') if isinstance(bal_resp, dict) else bal_resp
    ...
    for i in range(count):
        ...
        for amount in amounts:
            ...
            while attempt < max_attempts:
                attempt += 1
                try:
                    sig_resp = await brain._call_rpc('requestAirdrop', [str(pub), amount])
                    sig = None
                    if isinstance(sig_resp, dict):
                        sig = sig_resp.get('result') or sig_resp.get('value') or sig_resp.get('signature')
                    else:
                        sig = sig_resp
                    ...
                    ok = await wait_for_finalized(brain, sig, timeout=180)
                    if not ok:
                    ...
                    # success
                    new_bal_resp = await brain._call_rpc('getBalance', [str(pub)])
                    new_bal = new_bal_resp.get('result', {}).get('value') if isinstance(new_bal_resp, dict) else new_bal_resp
